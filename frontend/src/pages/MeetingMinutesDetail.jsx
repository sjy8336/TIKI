...
const TX = [
    {
        ...
        ts: 0,
        spk: '김지훈',
        txt: '안녕하세요, 오늘 Sprint 12 킥오프 회의 시작하겠습니다. AI 회의록 시스템 개발 현황 공유와 이번 스프린트 목표를 논의할 예정입니다.',
    },
    {
        time: '00:47',
        ts: 47,
        spk: '박소현',
        txt: '화자 분리 모델 쪽 진행 상황 먼저 공유할게요. 지난주에 Whisper 기반 STT와 PyAnnote를 연동하는 PoC를 완료했습니다.',
    },
    {
        time: '01:32',
        ts: 92,
        spk: '이민준',
        txt: 'Jira API 연동 PoC는 저번 주 목요일에 완료했습니다. POST /issue 엔드포인트로 업무 발행까지 잘 되는 것 확인했어요.',
    },
    {
        time: '02:15',
        ts: 135,
        spk: '최아로미',
        txt: '업로드 UI 드래그앤드롭 구현했고, 파일 유효성 검사(확장자/용량)도 프론트에서 처리하도록 했습니다. 사용자 피드백 수집이 다음 단계예요.',
    },
    {
        time: '03:01',
        ts: 181,
        spk: '김지훈',
        txt: 'STT 응답 속도가 현재 이슈인데, 평균 1분 오디오당 처리 시간이 8초 정도 나오고 있어요. 목표치 5초 대비 아직 최적화가 필요합니다.',
    },
    {
        time: '04:10',
        ts: 250,
        spk: '박소현',
        txt: '화자 분리 정확도는 2명 기준 94%, 4명 이상에서 79%로 떨어지는 문제가 있어요. 추가 학습 데이터가 필요할 것 같습니다.',
    },
    {
        time: '05:22',
        ts: 322,
        spk: '이민준',
        txt: '백엔드 task_id 폴링 방식은 현재 2초 간격으로 맞춰놨는데, 부하 테스트 결과 동시 요청 50개까지는 안정적이에요.',
    },
    {
        time: '06:45',
        ts: 405,
        spk: '최아로미',
        txt: '프론트엔드에서 단계별 진행 상황 UI는 완성됐습니다. 업로드 → 전처리 → AI 요약 → 업무 연동 4단계로 사용자에게 명확하게 보여주고 있어요.',
    },
    {
        time: '07:30',
        ts: 450,
        spk: '김지훈',
        txt: '배포 일정 이야기 해볼게요. 현재 계획은 6월 말인데, QA 기간을 1주 확보하려면 개발 완료를 6월 20일까지는 해야 합니다.',
    },
    {
        time: '08:15',
        ts: 495,
        spk: '박소현',
        txt: '화자 분리 정확도 개선 작업이 변수입니다. 추가 데이터 수집에 최소 3일은 걸릴 것 같아서 일정이 빠듯할 수 있어요.',
    },
    {
        time: '09:00',
        ts: 540,
        spk: '이민준',
        txt: '저는 일정 맞출 수 있을 것 같습니다. 다만 오류 핸들링 케이스가 아직 몇 가지 남아있는데, 이번 주 내로 처리할게요.',
    },
    {
        time: '10:20',
        ts: 620,
        spk: '김지훈',
        txt: '그럼 6월 20일 개발 완료, 6월 21-27일 QA, 6월 28일 배포로 최종 일정을 확정하겠습니다. 다들 동의하시죠?',
    },
];
...
function isAnonymousSpeakerLabel(value) {
    const normalized = normalizeSpeakerKey(value);
    if (!normalized) return true;
    const lowered = normalized.toLowerCase();
    return ['none', 'null', 'unknown', '미상', '기타'].includes(lowered) || /^speaker[\s_-]*\d+$/i.test(normalized);
}

function buildSpeakerDisplayMap(rows, analysisExtraData = null) {
    const map = new Map();
    const aliasBySpeakerId = new Map();
    let counter = 1;

    const diarization = analysisExtraData?.speaker_diarization || {};
    const mappingRows = Array.isArray(diarization?.speaker_participant_mapping)
        ? diarization.speaker_participant_mapping
        : [];
    const speakerStats = Array.isArray(diarization?.speaker_statistics) ? diarization.speaker_statistics : [];

    [...mappingRows, ...speakerStats].forEach((item) => {
        const speakerKey = normalizeSpeakerKey(
            item?.speaker_id ?? item?.speaker_alias ?? item?.speaker_label ?? item?.speaker ?? item?.who
        );
        if (!speakerKey || aliasBySpeakerId.has(speakerKey)) return;
        const aliasLabel = normalizeSpeakerKey(item?.speaker_alias ?? item?.speaker_label);
        if (aliasLabel) aliasBySpeakerId.set(speakerKey, aliasLabel);
    });

    const register = (speakerKey, preferredLabel) => {
        if (!speakerKey || map.has(speakerKey)) return;
        const label = normalizeSpeakerKey(preferredLabel);
        map.set(speakerKey, label ? label : `팀원 ${counter++}`);
    };
    ...
    (Array.isArray(rows) ? rows : []).forEach((item) => {
        ...
        const rawLabel = normalizeSpeakerKey(
            item?.speaker_display_name ??
                item?.participant_name ??
                item?.who ??
                item?.speaker_label ??
        ...
        register(speakerKey, aliasBySpeakerId.get(speakerKey) || (isAnonymousSpeakerLabel(rawLabel) ? '' : rawLabel));
    });

    aliasBySpeakerId.forEach((aliasLabel, speakerKey) => {
        if (!map.has(speakerKey)) {
            map.set(speakerKey, aliasLabel || `팀원 ${counter++}`);
        }
    });
        ...
        return items
            ...
            .filter((item) => {
                const projectMatches =
                    !normalizedProjectId || String(item?.projectId || currentProjectId) === normalizedProjectId;
            ...
            .flatMap((item, index) => {
                ...
                if (linkedSvcs.length === 0) {
                    const rawTool = String(
                        item?.integrationTool || item?.integrationProvider || item?.externalLink || item?.jiraLink || ''
                    ).toLowerCase();
...
function LucideIcon({ name, size = 14, color = 'currentColor', strokeWidth = 2, className = '' }) {
    ...
    switch (name) {
        ...
        case 'star':
            return (
                <svg {...common}>
                    <path d="m12 3.5 2.8 5.67 6.25.9-4.52 4.41 1.07 6.22L12 17.76 6.4 20.7l1.07-6.22L2.95 10.07l6.25-.9L12 3.5Z" />
                </svg>
            );
        case 'star-filled':
            return (
        ...
        case 'play':
            return (
                <svg {...common}>
                    <path d="m8 6 10 6-10 6V6Z" fill={color} stroke="none" />
                </svg>
            );
        case 'pause':
            return (
...
function Modal({ open, onClose, title, children, footer, maxWidth = 448, bodyOverflowY = 'auto', bodyHeight }) {
    ...
    return (
        ...
        >
            ...
            />
            <div
                className="relative self-center w-full rounded-2xl bg-white border border-[rgba(0,100,180,0.12)] shadow-[0_18px_50px_rgba(13,27,42,0.22)]"
                style={{ maxWidth }}
            >
                <div className="flex items-center justify-between px-5 py-4 border-b border-[rgba(0,100,180,0.08)]">
                    ...
                    <button
                        type="button"
                        onClick={onClose}
                        className="w-8 h-8 rounded-lg hover:bg-[#F8FAFF] text-[#5A6F8A] flex items-center justify-center"
                    >
                        <LucideIcon name="x" size={16} />
                    </button>
                </div>
                <div className="px-5 py-4" style={{ overflowY: bodyOverflowY, height: bodyHeight }}>
                    {children}
                </div>
                {footer && (
                    <div className="px-5 py-4 border-t border-[rgba(0,100,180,0.08)] flex items-center justify-end gap-2">
                        {footer}
                    </div>
                )}
            </div>
...
function CustomDropdown({ value, onChange, options, placeholder, disabled = false, hasError = false, triggerStyle }) {
                    ...
                    {options.map((option) => (
                        <button
                            ...
                            className={`w-full text-left px-3 py-2 text-sm transition-colors cursor-pointer ${
                                option === value ? 'bg-cyan-50 text-cyan-700' : 'text-slate-700 hover:bg-slate-50'
                            }`}
                        >
                            {option}
                        </button>
...
function ServiceDetailModal({ open, onClose, svc, auditLog }) {
    ...
    const displayTickets = [
        ...svc.tickets.map((ticket) => {
            const isIssued =
                issuedTicketTitles.has(normalizeText(ticket.title)) ||
                issuedTicketTitles.has(normalizeText(`${ticket.id} · ${ticket.title}`));
            ...
            <div className="mb-4">
                <div className="flex justify-between mb-1.5">
                    ...
                    <span className="text-xs font-bold text-slate-900">
                        {hasIssued ? `${svcLogs.length}건 연동` : '연동 전'}
                    </span>
                </div>
            </div>
            ...
            {!hasIssued ? (
                <div className="rounded-xl p-4 mb-4 border border-slate-200 bg-slate-50">
                    ...
                    <p className="text-xs text-slate-400 mt-1">
                        업무 보내기를 실행하면 이곳에 실제 발행 이력이 표시됩니다.
                    </p>
                </div>
            ) : (
        ...
        >
            ...
            <div>
                ...
                <div ref={duePickerRef} className="relative" data-due-picker-root>
                    ...
                    >
                        <span className={`${due && due !== '미정' ? 'text-slate-700' : 'text-slate-400'}`}>
                            {due || '마감일'}
                        </span>
                        <LucideIcon name="chevron-down" size={12} color={PROJECTLIST_CHEVRON_COLOR} strokeWidth={2} />
                    </button>
...
function IssueModal({ open, onClose, onIssued, services, isMobile, assigneeOptions = [] }) {
    ...
    const selectableItemIndexes = useMemo(
        () =>
            ACTION_ITEMS_FOR_ISSUE.map((item, idx) => (isIssueItemIssued(item) ? null : idx)).filter(
                (idx) => idx !== null
            ),
        [isIssueItemIssued]
    );
    ...
    const goStep2 = () => {
        ...
        const autoTitle = items.length === 1 ? items[0].text : `${items[0].text} 외 ${items.length - 1}건`;
        ...
        const defaultMergedDue = datedCandidates.length > 0 ? datedCandidates[0].label : dueCandidates[0] || '미정';

        setTitle(autoTitle);
        setAssignee(
            items.length === 1 && items[0].assignee && items[0].assignee !== '미정'
                ? items[0].assignee
                : assigneeOptions[0] || ''
        );
    ...
    const handleIssue = async () => {
        ...
        const targetSvcIds =
            selectedSvc === 'both' ? ['jira', 'notion'] : [selectedSvc === 'notion' ? 'notion' : 'jira'];
    ...
    const selectedSvcObj =
        selectedSvc === 'both'
            ? { id: 'both', name: 'Jira + Notion', iconBg: '#7C3AED', iconLabel: 'J+N' }
            : services.find((s) => s.id === selectedSvc);
    const selectedSvcIds = selectedSvc === 'both' ? ['jira', 'notion'] : selectedSvc ? [selectedSvc] : [];
    const toggleSvc = (svcId) => {
        setSelectedSvc((prev) => {
            const current = prev === 'both' ? ['jira', 'notion'] : prev ? [prev] : [];
            const next = current.includes(svcId) ? current.filter((id) => id !== svcId) : [...current, svcId];
                                ...
                                return (
                                    <button
                                        ...
                                        style={{
                                            ...
                                            background: isSelected ? 'rgba(0,153,204,0.06)' : '#fff',
                                        }}
                                    >
                                        <span
                    ...
                    <div>
                        <div className="mb-2.5 flex items-center justify-between gap-3">
                            <p className="text-xs font-semibold text-slate-400 uppercase tracking-widest">
                                보낼 업무 선택
                                {checkedItems.size > 0 && (
                                    <span className="ml-2 text-cyan-500 normal-case">
                                        ({checkedItems.size}개 선택됨)
                                    </span>
                                )}
                            </p>
                                    ...
                                    >
                                        <div
                                            ...
                                            style={{
                                                ...
                                                borderColor:
                                                    alreadyIssued || checked ? '#10B981' : 'rgba(0,100,180,0.2)',
                                            }}
                                        >
                                            {(alreadyIssued || checked) && (
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <p className="text-xs text-slate-400 mt-0.5">
                                                {item.assignee || '미정'} · {getDueLabel(idx)}
                                            </p>
                                        </div>
                                    </div>
                                );
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                ...
                                <div className="min-w-0 col-span-2">
                                    ...
                                    <div ref={mergedDuePickerRef} className="relative" data-due-picker-root>
                                        ...
                                        >
                                            <span
                                                className={`${mergedDue && mergedDue !== '미정' ? 'text-slate-700' : 'text-slate-400'}`}
                                            >
                                                {mergedDue || '마감일'}
                                            </span>
                                            <LucideIcon
                                                name="chevron-down"
                                                size={12}
                                                color={PROJECTLIST_CHEVRON_COLOR}
                                                strokeWidth={2}
                                            />
                                        </button>
        ...
        <Modal
            ...
            footer={
                <>
                    <button
                        onClick={onClose}
                        className="text-sm font-semibold px-4 py-2 rounded-xl text-slate-500 hover:bg-slate-100"
                    >
                        취소
                    </button>
                    <button
...
/* ─── Transcript Card ────────────────────────────────── */
function TxCard({ item, isActive, isBookmarked, isCollapsed, onSeek, onToggleBm, speakerDisplayMap }) {
    const speakerKey = normalizeSpeakerKey(
        item?.speaker_id ??
            item?.speaker ??
            item?.spk ??
    ...
    const anonLabel = speakerDisplayMap?.get(speakerKey) || rawLabel;
    const anonInitial = String(anonLabel).replace(/^팀원\s*/, '') || '?';
    const col = SPK_COLOR[speakerKey] || SPK_COLOR[rawLabel] || '#5A6F8A';
    ...
    return (
        ...
        >
            <div className="flex items-center gap-3 px-4 py-3 cursor-pointer" onClick={() => onSeek(item.ts)}>
                <span
                    className={`text-xs font-semibold w-10 flex-shrink-0 ${isActive ? 'text-cyan-500' : 'text-slate-400'}`}
                >
                    {item.time}
                </span>
                <div
                ...
                <p className="text-xs font-semibold flex-1 min-w-0" style={{ color: col }}>
                    {anonLabel}
                </p>
    ...
    return (
        ...
        >
            ...
            <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-slate-400 mt-0.5">
                    {item.assignee || '미정'}
                    {checked ? ' · 완료' : item.due ? ` · ${normalizeDueLabel(item.due) || item.due}` : ''}
                </p>
            </div>
            {checked && (
...
function SummaryPanel({
    ...
    const [decisions, setDecisions] = useState(
        (summaryData.decisions || []).map((item) => normalizeFullDateLabel(item))
    );
    ...
    const buildDecisions = useCallback(() => {
        return decisionsDraft.map((v) => normalizeFullDateLabel(v || '').trim()).filter(Boolean);
    }, [decisionsDraft]);
    ...
    const buildNextAgenda = useCallback(() => {
        return nextAgendaDraft.map((v) => formatNextAgendaItem(v, true)).filter(Boolean);
    }, [nextAgendaDraft]);
    ...
    return (
        <div
            className={`bg-white border border-slate-200 rounded-2xl overflow-hidden flex flex-col ${transcriptVisible ? 'h-full' : 'h-auto'}`}
        >
                ...
                <div className="flex items-center gap-2 relative">
                    {!isMobile && transcriptEnabled && (
                        ...
                        >
                            <svg
                                ...
                                fill="none"
                                style={{
                                    transform: transcriptVisible ? 'rotate(0deg)' : 'rotate(180deg)',
                                    transition: 'transform 0.3s ease',
                                }}
                            >
                                <path
                                    d="M9 2H12M9 5H12M9 8H12M2 11H12M2 2H6V8H2V2Z"
                                    stroke="currentColor"
                                    strokeWidth="1.4"
                                    strokeLinecap="round"
                                    strokeLinejoin="round"
                            ...
                            <span className="hidden sm:inline text-[14px] leading-none">
                                {transcriptVisible ? '스크립트 접기' : '스크립트 펼치기'}
                            </span>
                        </button>
                    )}
                    ...
                    {isMobile && (
                        ...
                        >
                            <span
                                style={{
                                    transform: summaryCollapsed ? 'rotate(180deg)' : 'rotate(0deg)',
                                    transition: 'transform 0.2s ease',
                                }}
                            >
                                <LucideIcon
                        ...
                        <>
                                        ...
                                        <div className="flex flex-wrap gap-1.5 mt-2.5">
                                            {keywordsText
                                                ...
                                                .map((k, i) => (
                                                    <span
                                                        key={`${k}-${i}`}
                                                        className={`text-xs font-semibold px-2.5 py-1 rounded-full ${KW_BADGE}`}
                                                    >
                                                        {k}
                                                    </span>
                                                ))}
                            ...
                            <div>
                                ...
                                <div className="space-y-2">
                                    {decisionsDraft.length === 0 && (
                                        <EditEmptyRow>아직 추가된 결정 사항이 없어요</EditEmptyRow>
                                    )}
                                    {decisionsDraft.map((item, idx) => (
                                        <div
                                            key={`decision-edit-${idx}`}
                                        ...
                                        >
                                            ...
                                            <input
                                                ...
                                                onBlur={(e) =>
                                                    setDecisionsDraft((prev) =>
                                                        prev.map((v, i) =>
                                                            i === idx ? normalizeFullDateLabel(e.target.value) : v
                                                        )
                                                    )
                                                }
                                                className={EDIT_INPUT_CLS}
                                            ...
                                            <EditRemoveButton
                                                onClick={() =>
                                                    setDecisionsDraft((prev) => prev.filter((_, i) => i !== idx))
                                                }
                                            />
                                        </div>
                                    ))}
                            ...
                            <div>
                                        ...
                                        <EditAddButton
                                            onClick={() =>
                                                setActionsDraft((prev) => [
                                                    ...prev,
                                                    { text: '', assignee: '', due: null, status: 'todo' },
                                                ])
                                            }
                                        />
                                ...
                                <div className="space-y-2.5">
                                    {actionsDraft.length === 0 && (
                                        <EditEmptyRow>아직 추가된 할 일이 없어요</EditEmptyRow>
                                    )}
                                        ...
                                        return (
                                            ...
                                            >
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    <EditRemoveButton
                                                        onClick={() =>
                                                            setActionsDraft((prev) => prev.filter((_, i) => i !== idx))
                                                        }
                                                    />
                                                </div>
                                                <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                                                    <input
                                                        ...
                                                        onChange={(e) =>
                                                            setActionsDraft((prev) =>
                                                                prev.map((v, i) =>
                                                                    i === idx ? { ...v, assignee: e.target.value } : v
                                                                )
                                                            )
                                                        }
                                                    ...
                                                    <div className="relative" data-due-picker-root>
                                                        ...
                                                        >
                                                            <span
                                                                className={`${item.due ? 'text-slate-700' : 'text-slate-400'}`}
                                                            >
                                                                {item.due || '마감일'}
                                                            </span>
                                                            <LucideIcon
                            ...
                            <div>
                                <div className="flex items-center justify-between mb-3">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span className="text-xs font-semibold text-slate-400">
                                            {issuesDraft.length}건
                                        </span>
                                    </div>
                                    <div className="flex items-center gap-2">
                                        <EditAddButton
                                            onClick={() =>
                                                setIssuesDraft((prev) => [...prev, { level: 'medium', text: '' }])
                                            }
                                        />
                                        ...
                                        return (
                                            ...
                                            >
                                                ...
                                                <div className="self-end sm:self-auto">
                                                    <EditRemoveButton
                                                        onClick={() =>
                                                            setIssuesDraft((prev) => prev.filter((_, i) => i !== idx))
                                                        }
                                                    />
                                                </div>
                                            </div>
                            ...
                            <div>
                                <div className="flex items-center justify-between mb-3">
                                    <div className="flex items-center gap-2">
                                        ...
                                        <span className="text-xs font-semibold text-slate-400">
                                            {nextAgendaDraft.length}건
                                        </span>
                                    </div>
                                    <div className="flex items-center gap-2">
                                        <EditAddButton
                                            onClick={() => setNextAgendaDraft((prev) => [...prev, { topic: '' }])}
                                        />
                                        <EditSaveButton onClick={() => handleSaveSection('nextAgenda')} />
                                    </div>
                                </div>
                                <div className="space-y-2">
                                    {nextAgendaDraft.length === 0 && (
                                        <EditEmptyRow>다음 회의 안건을 추가해보세요</EditEmptyRow>
                                    )}
                                        ...
                                        <div key={`agenda-edit-${idx}`} className="flex items-start gap-2">
                                            ...
                                            <div className="flex-1">
                                                <input
                                                    value={item.topic || ''}
                                                    onChange={(e) =>
                                                        setNextAgendaDraft((prev) =>
                                                            prev.map((v, i) =>
                                                                i === idx ? { ...v, topic: e.target.value } : v
                                                            )
                                                        )
                                                    }
                                                    onBlur={(e) =>
                                                        setNextAgendaDraft((prev) =>
                                                            prev.map((v, i) =>
                                                                i === idx
                                                                    ? {
                                                                          ...v,
                                                                          topic: normalizeFullDateLabel(e.target.value),
                                                                      }
                                                                    : v
                                                            )
                                                        )
                                            ...
                                            <div className="self-center">
                                                <EditRemoveButton
                                                    onClick={() =>
                                                        setNextAgendaDraft((prev) => prev.filter((_, i) => i !== idx))
                                                    }
                                                />
                                            </div>
                                        </div>
                        ...
                        <>
                            ...
                            <div>
                                <CollapsibleSectionHeader
                                    ...
                                    badge={<span className="text-xs text-slate-300">{decisions.length}건</span>}
                                />
                                <div
                                    style={{
                                ...
                                <CollapsibleSectionHeader
                                    ...
                                    badge={
                                        <span className="text-xs text-slate-300">
                                            {doneActionsCount}/{actions.length}
                                        </span>
                                    }
                                />
                                <div
                                        ...
                                        {summaryData.issues.map((iss, i) => {
                                            ...
                                            return (
                                                ...
                                                >
                                                    ...
                                                    <span
                                                        className={`flex-shrink-0 text-xs font-bold px-2 py-0.5 rounded ${c.badge}`}
                                                    >
                                                        {c.label}
                                                    </span>
                                                </div>
                            ...
                            <div>
                                <CollapsibleSectionHeader
                                    ...
                                    badge={
                                        <span className="text-xs text-slate-300">
                                            {summaryData.next_agenda.length}건
                                        </span>
                                    }
                                />
                                <div
                                        ...
                                        {summaryData.next_agenda.map((item, i) => (
                                            <div key={i} className="flex items-start gap-2.5">
                                                ...
                                                <p className="text-sm text-slate-800 leading-relaxed">
                                                    {isMobile
                                                        ? normalizeMobileAgendaDateLabel(item)
                                                        : normalizeInlineAgendaDateLabel(item)}
                                                </p>
                                            </div>
                                        ))}
                                    </div>
...
function SummaryEditModal({ open, onClose, summaryData, onSave }) {
    ...
    useEffect(() => {
        ...
        setIssuesText((summaryData.issues || []).map((i) => `${i.level}: ${i.text}`).join('\n'));
    ...
    const handleSave = () => {
        ...
        const actions = actionsText
            ...
            .map((line) => {
                ...
                const normalizedStatus = statusPart === 'done' || statusPart === '완료' ? 'done' : 'todo';
            ...
            <div className="space-y-4">
                ...
                <div>
                    <p className="text-xs font-semibold text-slate-400 mb-1.5">
                        해야 할 일 (한 줄당: 내용 | 담당자 | 마감일 | 상태(todo/done))
                    </p>
                    <textarea
                        rows={5}
                        value={actionsText}
                ...
                <div>
                    <p className="text-xs font-semibold text-slate-400 mb-1.5">
                        이슈/리스크 (한 줄당: high|medium|low: 내용)
                    </p>
                    <textarea
                        rows={4}
                        value={issuesText}
    ...
    return (
        ...
        >
            <div className="max-w-screen-xl mx-auto flex items-center gap-3 md:gap-4">
                ...
                >
                    {playing ? (
                        <LucideIcon name="pause" size={14} color="#fff" />
                    ) : (
                        <LucideIcon name="play" size={14} color="#fff" />
                    )}
                </button>
                    ...
                    <div className="flex items-center gap-1.5 flex-shrink-0">
                        ...
                        <span className="text-xs text-slate-400">
                            ...
                            <span className="font-semibold text-slate-600">
                                {fmtAuditTime(latestGeneratedLog.time)}
                            </span>
                        </span>
                    </div>
                )}
...
export default function TikiSprint12() {
    ...
    const [summaryActions, setSummaryActions] = useState(() => SUMMARY_DATA.actions.map((action) => ({ ...action })));
    ...
    const [analysisExtraData, setAnalysisExtraData] = useState(null);
    ...
    const speakerDisplayMap = useMemo(() => buildSpeakerDisplayMap(txSource, analysisExtraData), [txSource, analysisExtraData]);
    ...
    useEffect(() => {
        ...
        if (
            state.meetingId &&
            state.projectId &&
            (searchParams.get('meetingId') !== state.meetingId || searchParams.get('projectId') !== state.projectId)
        ) {
        ...
        const normalizeAction = (item) => ({
            ...
            status:
                item?.status === '수행완료' || item?.status === '완료' || item?.status === 'done' || item?.checked
                    ? 'done'
                    : 'todo',
        });
            ...
            try {
                ...
                if (!match) {
                    ...
                    const visibleItems = rawItems.filter(
                        (item) => !(item?.__tiki_meta || item?.type === '__tiki_meeting_meta')
                    );
                        ...
                        keywords:
                            Array.isArray(metaData.keywords) && metaData.keywords.length
                                ...
                                : // Older manually-written meetings (before the meta marker existed)
                                  ...
                                  ? meeting.tags
                                        .map((tag) => ({ text: String(tag || '').replace(/^#/, ''), type: 'cyan' }))
                                        .filter((k) => k.text)
                                  : [],
                        decisions:
                            Array.isArray(metaData.decisions) && metaData.decisions.length
                                ? metaData.decisions
                                      .map((d) => (typeof d === 'string' ? d : d?.text || ''))
                                      .filter(Boolean)
                                : [],
                        issues:
                            Array.isArray(metaData.issues) && metaData.issues.length
                                ? metaData.issues
                                      .map((i) => ({
                                          level: priorityToLevel[i?.priority] || 'medium',
                                          text: i?.text || '',
                                      }))
                                      .filter((i) => i.text)
                                : [],
                        next_agenda:
                            typeof metaData.nextAgenda === 'string' && metaData.nextAgenda.trim()
                                ? metaData.nextAgenda
                                      .split('\n')
                ...
                setAnalysisExtraData(analysis.extra_data || null);
                setSummaryData((prev) => ({
                    ...prev,
                    ...
                    keywords:
                        Array.isArray(analysis.keywords) && analysis.keywords.length
                            ...
                            : prev.keywords,
                    decisions:
                        Array.isArray(analysis.decisions) && analysis.decisions.length
                            ? analysis.decisions
                            : prev.decisions,
                    issues: Array.isArray(analysis.issues) && analysis.issues.length ? analysis.issues : prev.issues,
                    next_agenda:
                        Array.isArray(analysis.next_agenda) && analysis.next_agenda.length
                            ? analysis.next_agenda
                            : prev.next_agenda,
                }));
                ...
                const liveActionItems = Array.isArray(liveMeeting?.action_items)
                    ? liveMeeting.action_items.filter(
                          (item) => !(item?.__tiki_meta || item?.type === '__tiki_meeting_meta')
                      )
                    : [];
                ...
                if (!cancelled) setRealDataStatus('missing');
                if (!cancelled) setAnalysisExtraData(null);
            } finally {
    ...
    const uploadKind = useMemo(() => {
        ...
        const hasTextDirect = directCandidates.some((v) =>
            ['text', 'transcript', 'document', 'doc', 'file'].includes(v)
        );
        ...
        if (
            mimeCandidates.some(
                (v) => v.startsWith('text/') || v.includes('pdf') || v.includes('word') || v.includes('document')
            )
        )
            return 'text';
        ...
        (svcName, issuedItems = []) => {
            const svcIds = Array.isArray(svcName)
                ? svcName
                      .map(
                          (name) =>
                              INITIAL_INTEGRATION_SERVICES.find((s) => s.name === name)?.id ||
                              String(name).toLowerCase()
                      )
                      .filter((id) => id === 'jira' || id === 'notion')
            ...
            setAuditLog((prev) => [
                ...prev,
                ...logs.map((item) => ({ svcId: item.svcId, label: item.label, time: timeStr, user: item.user })),
            ]);
            ...
            if (projectId && sourceTitle && logs.length > 0) {
                const overrides = readProjectOverrides();
                const prevProject =
                    overrides[projectId] && typeof overrides[projectId] === 'object' ? overrides[projectId] : {};
                ...
                logs.forEach((item, index) => {
                    ...
                    const existingLinks =
                        existing?.integrationLinks && typeof existing.integrationLinks === 'object'
                            ? existing.integrationLinks
                            : {};
    ...
    const txData = txSource
        ...
        .filter(
            (d) =>
                ...
                (!searchQ ||
                    String(d.txt || '').includes(searchQ) ||
                    String(d.spk || '').includes(searchQ) ||
                    String(d.who || '').includes(searchQ) ||
                    String(d.participant_name || '').includes(searchQ) ||
                    String(d.speaker_display_name || '').includes(searchQ) ||
                    String(d.speaker_label || '').includes(searchQ))
        );
    ...
    return (
        ...
        >
                    ...
                    <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-5">
                        <div className="flex-1 min-w-0">
                            ...
                            {realDataStatus === 'loaded' && analysisDegraded && (
                                <div className="mb-3 rounded-lg bg-amber-50 border border-amber-200 px-3 py-2 text-xs font-semibold text-amber-700">
                                    AI 분석이 일시적으로 실패해 간이 자동 요약으로 대체되었습니다. 내용을 꼭 검토해
                                    주세요.
                                </div>
                            )}
                            <div className="flex flex-wrap items-center gap-2 mb-2">
                                <span className="text-xs font-semibold text-slate-400">
                                    {meetingHeader?.date || '2026.06.14'}
                                </span>
                            </div>
                                ...
                                <div className="flex -space-x-2">
                                    {visibleParticipants.map((name, idx) => (
                                        <div
                                            ...
                                            style={{
                                                background: PARTICIPANTS[idx % PARTICIPANTS.length]?.color || '#0099CC',
                                            }}
                                            title={name}
                                        >
                                            {name[0] || '?'}
            ...
            <div className={`px-4 md:px-8 lg:px-12 py-5 max-w-screen-xl mx-auto ${isMobile ? 'pb-[230px]' : 'pb-32'}`}>
                <div
                                    ...
                                    <div className="flex items-center justify-between mb-4 gap-3 flex-wrap">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <button
                                                ...
                                                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-medium border transition-colors cursor-pointer ${
                                                    bmFilter
                                                        ...
                                                        : 'bg-white text-slate-400 border-slate-200 hover:bg-blue-50'
                                                }`}
                                            >
                                                <LucideIcon name={bmFilter ? 'star-filled' : 'star'} size={13} />{' '}
                                            ...
                                            >
                                                <span
                                                    style={{
                                                        transform: allCollapsed ? 'rotate(180deg)' : 'rotate(0deg)',
            ...
            <RegenModal
                ...
                onRegen={({ focus, prompt, len }) => {
                    setSummaryData((prev) => {
                        ...
                        const secondAction =
                            todoActions[1]?.text || doneActions[0]?.text || '담당자 기준으로 실행 계획을 조정했습니다.';

                        const seed = [focusSentences[0], focusSentences[1], baseSentences[0], baseSentences[1]].filter(
                            Boolean
                        );
                        ...
                        const conciseLines = [
                            ...
                            userPrompt ? `핵심 실행 항목: ${firstAction}` : `핵심 실행 항목: ${firstAction}`,
                        ];
