...
function Icon({ name, size = 20, className = "" }) {
  const icons = {
    sparkles: <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={className}><path d="m12 3-1.912 5.813a2 2 0 0 1-1.275 1.275L3 12l5.813 1.912a2 2 0 0 1 1.275 1.275L12 21l1.912-5.813a2 2 0 0 1 1.275-1.275L21 12l-5.813-1.912a2 2 0 0 1-1.275-1.275L12 3Z"/><path d="m5 3 1 2.5L8.5 6 6 7 5 9.5 4 7 1.5 6 4 5 5 3Z"/><path d="m19 17 1 2.5 2.5.5-2.5 1-1 2.5-1-2.5-2.5-1 2.5-1 1-2.5Z"/></svg>,
    mic: <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={className}><path d="M12 2a3 3 0 0 0-3 3v7a3 3 0 0 0 6 0V5a3 3 0 0 0-3-3Z"/><path d="M19 10v2a7 7 0 0 1-14 0v-2"/><line x1="12" y1="19" x2="12" y2="22"/></svg>,
    ...
    link: <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={className}><path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71"/><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71"/></svg>,
    upload: <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={className}><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg>,
...
export default function TikiLandingPage() {
  ...
  const roadmap = [
    {
      phase: "v1.0", label: "현재 운영 중", status: "done",
      color: "#10B981", bg: "from-[#10B981]/10 to-[#10B981]/5", border: "border-[#10B981]/30",
      items: ["사후 오디오 업로드 분석", "LLM 액션 아이템 추출", "Jira 원클릭 연동", "보안 마스킹 시스템"],
    },
    {
      phase: "v1.5", label: "다음 분기 예정", status: "progress",
      color: "#0099CC", bg: "from-[#0099CC]/10 to-[#0099CC]/5", border: "border-[#0099CC]/30",
      items: ["실시간 대화 스트리밍 요약", "Slack 알림 자동 발송", "Notion 데이터베이스 연동"],
    },
    {
      phase: "v2.0", label: "하반기 목표", status: "planned",
      color: "#7C3AED", bg: "from-[#7C3AED]/10 to-[#7C3AED]/5", border: "border-[#7C3AED]/30",
    ...
    <div className="min-h-screen bg-[#F8FAFF] text-[#0D1B2A] font-['Pretendard',sans-serif] antialiased overflow-x-hidden">
      ...
      <section className="relative pt-28 pb-24 lg:pt-40 lg:pb-32 overflow-hidden">
        ...
        <div className="absolute inset-0 opacity-[0.03]"
          style={{ backgroundImage: "linear-gradient(rgba(0,100,180,1) 1px, transparent 1px), linear-gradient(90deg, rgba(0,100,180,1) 1px, transparent 1px)", backgroundSize: "60px 60px" }} />
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <h1 className="text-4xl sm:text-6xl lg:text-7xl font-extrabold tracking-tight text-[#0D1B2A] leading-[1.1] mb-6">
            회의록 작성에<br className="hidden sm:block" /> 쏟는 시간,<br />
            ...
            <div className="relative bg-white rounded-2xl border border-[rgba(0,100,180,0.12)] shadow-2xl overflow-hidden">
              ...
              <div className="p-4 sm:p-6 bg-[#F8FAFF]">
                ...
                <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-4 p-4 bg-white rounded-xl border border-[rgba(0,100,180,0.12)] gap-3">
                  <div>
                    <div className="flex items-center gap-1.5 mb-1">
                      <span className="w-2 h-2 rounded-full bg-[#10B981] animate-pulse" />
                      <span className="text-[10px] font-bold text-[#10B981] uppercase tracking-wide">AI 분석 완료</span>
                    </div>
                    <p className="text-sm font-bold text-[#0D1B2A]">네오테크 6월 3주차 스프린트 회의</p>
                    <p className="text-xs text-[#5A6F8A] mt-0.5">참여자 4명 · 녹화 시간 38분 · 액션아이템 7개 추출</p>
                  </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
                  {[
                    { status: "검증 전", title: "컬러 팔레트 CSS 변수 정의", assignee: "정아름", time: "03:12", priority: "보통", statusColor: "bg-[#F59E0B]", priorityColor: "text-[#F59E0B]" },
                    { status: "Jira 연동됨", title: "세션 토큰 리프레시 로직 보완", assignee: "김소현", time: "11:45", priority: "높음", statusColor: "bg-[#10B981]", priorityColor: "text-[#EF4444]" },
                    { status: "Jira 연동됨", title: "Figma 최종 시안 업로드 요청", assignee: "채하율", time: "24:02", priority: "낮음", statusColor: "bg-[#10B981]", priorityColor: "text-[#5A6F8A]" },
                  ].map((card, i) => (
                    <div key={i} className="bg-white rounded-xl border border-[rgba(0,100,180,0.12)] p-4 hover:border-[rgba(0,153,204,0.3)] transition-colors text-left">
                      <div className="flex justify-between items-center mb-3">
                        <span className={`text-[10px] font-bold text-white px-2 py-0.5 rounded-full ${card.statusColor}`}>
                          {card.status}
                        </span>
                        <span className={`text-[10px] font-semibold ${card.priorityColor}`}>{card.priority}</span>
                      </div>
                      <p className="text-xs font-bold text-[#0D1B2A] leading-snug mb-3">{card.title}</p>
                      <div className="flex justify-between text-[10px] text-[#5A6F8A] pt-2 border-t border-gray-100">
                        <span className="font-semibold inline-flex items-center gap-1">
                          <User size={11} className="text-[#5A6F8A]" />
                          {card.assignee}
                        </span>
                        <span className="text-[#0099CC] font-bold inline-flex items-center gap-1">
                          <Clock3 size={11} className="text-[#0099CC]" />
                          {card.time}
                        </span>
                      </div>
                    </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="max-w-3xl mx-auto text-center mb-14">
            ...
            <h2 className="text-3xl sm:text-4xl lg:text-5xl font-extrabold text-[#0D1B2A] mb-4 leading-tight">
              지금도 이런 상황이<br />반복되고 있진 않나요?
            </h2>
            <p className="text-[#5A6F8A] text-base sm:text-lg">회의 후 매번 30분을 낭비하는 팀을 위해 TIKI가 설계되었습니다.</p>
          </div>
            ...
            {problems.map((item, i) => {
              ...
              <div key={i}
                ...
                <span className="shrink-0 mt-0.5 w-9 h-9 rounded-xl bg-[rgba(0,100,180,0.06)] flex items-center justify-center">
                  <ProblemIcon size={20} className="text-[#5A6F8A] group-hover:text-[#0D1B2A] transition-colors" />
                </span>
                <p className="text-sm text-[#5A6F8A] leading-relaxed group-hover:text-[#0D1B2A] transition-colors">{item.text}</p>
              </div>
            );})}
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="max-w-2xl mx-auto text-center mb-16">
            <span className="inline-flex items-center gap-1.5 text-xs font-bold text-[#0099CC] bg-[#0099CC]/10 px-3 py-1.5 rounded-full uppercase tracking-widest mb-4">
              <Settings2 size={12} className="text-[#0099CC]" />
              작동 방식
            </span>
          ...
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8 max-w-5xl mx-auto">
            {steps.map((step, i) => (
              <div key={i}
                ...
                style={activeStep === i ? { borderColor: step.color, boxShadow: `0 20px 40px ${step.color}25` } : {}}>
                <div className="text-xs font-black mb-3 tracking-widest" style={{ color: activeStep === i ? step.color : "#5A6F8A" }}>
                  {step.num}
                </div>
                ...
                {/* 활성 인디케이터 */}
                {activeStep === i && (
                  <div className="absolute bottom-0 left-0 right-0 h-1.5" style={{ backgroundColor: step.color }} />
                )}
              </div>
            ))}
          </div>
          ...
          <div className="max-w-5xl mx-auto p-6 bg-[#F8FAFF] rounded-2xl border border-[rgba(0,100,180,0.1)]">
            <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4">
              ...
              <div className="flex-1">
                <p className="text-xs font-bold text-[#5A6F8A] uppercase tracking-widest mb-1">STEP {activeStep + 1}</p>
                ...
                {steps.map((_, i) => (
                  <button key={i} onClick={() => setActiveStep(i)}
                    ...
                    style={{
                      ...
                      backgroundColor: i === activeStep ? steps[activeStep].color : "rgba(0,100,180,0.2)",
                    }} />
                ))}
              </div>
            </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="max-w-2xl mx-auto text-center mb-16">
            <span className="inline-flex items-center gap-1.5 text-xs font-bold text-[#7C3AED] bg-[#7C3AED]/10 px-3 py-1.5 rounded-full uppercase tracking-widest mb-4">
              <LucideSparkles size={12} className="text-[#7C3AED]" />
              핵심 기능
            </span>
            <h2 className="text-3xl sm:text-4xl lg:text-5xl font-extrabold text-[#0D1B2A] mb-4">
              기능이 아닌, 결과를<br />경험하세요
            </h2>
            <p className="text-[#5A6F8A] text-base sm:text-lg">TIKI는 기술 스펙을 자랑하지 않습니다. 팀이 실제로 체감하는 변화를 만듭니다.</p>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5">
            {features.map((f, i) => (
              <div key={i}
                ...
                {/* 배경 그라디언트 */}
                <div className={`absolute inset-0 bg-gradient-to-br ${f.gradient} opacity-0 group-hover:opacity-100 transition-opacity rounded-2xl`} />

                <div className="relative">
                  {/* 배지 */}
                  <div className="inline-flex items-center gap-1 px-2 py-1 rounded-lg text-[10px] font-bold mb-5"
                  ...
                  <p className="text-xs font-semibold mb-3" style={{ color: f.color }}>{f.sub}</p>
      ...
      <section id="why" className="py-20 sm:py-28 bg-white relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="max-w-2xl mx-auto text-center mb-16">
            ...
            <p className="text-[#5A6F8A] text-base sm:text-lg">기능은 비슷해 보여도 핵심 철학이 다릅니다.</p>
          </div>

          <div className="max-w-3xl mx-auto grid grid-cols-1 sm:grid-cols-2 gap-5">
            {whyItems.map((col, ci) => (
              <div key={ci}
                className={`rounded-2xl border p-6 transition-all ${ci === 0
                ...
                <div className="flex items-center gap-2 mb-6">
                  {ci === 0
                    ...
                    : <span className="px-3 py-1.5 bg-[#5A6F8A]/10 text-[#5A6F8A] text-xs font-bold rounded-full">타 서비스</span>
                  }
                </div>
                  ...
                  {col.items.map((item, ii) => (
                    <li key={ii} className="flex items-start gap-3 text-sm">
                      <span className={`shrink-0 mt-0.5 w-5 h-5 rounded-full flex items-center justify-center ${
                        col.isUs ? "bg-[#10B981]/15 text-[#10B981]" : "bg-[#EF4444]/10 text-[#EF4444]"
                      }`}>
                        {col.isUs ? <Icon name="check" size={11} /> : <Icon name="x" size={11} />}
                      </span>
                      <span className={col.isUs ? "text-[#0D1B2A] font-medium" : "text-[#5A6F8A]"}>{item}</span>
                    </li>
                  ))}
                </ul>
              </div>
      ...
      <section id="tech" className="py-20 sm:py-28 relative overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 max-w-3xl mx-auto mb-10">
            {techStack.map((t, i) => (
              <div key={i}
                ...
                <div className="w-11 h-11 rounded-xl mx-auto mb-3 flex items-center justify-center text-xs font-extrabold transition-all group-hover:scale-110"
          ...
          <div className="max-w-3xl mx-auto p-5 bg-white/5 border border-white/10 rounded-2xl">
            <p className="text-xs font-bold text-white/40 mb-4 uppercase tracking-widest">데이터 아키텍처 흐름</p>
            <div className="flex flex-wrap items-center gap-2 text-xs">
              {["오디오 수신", "Whisper STT", "LangChain LLM", "Redis 캐싱", "PostgreSQL", "Supabase 인증", "Jira · Notion"].map((item, i, arr) => (
                <React.Fragment key={i}>
                  <span className="px-3 py-1.5 bg-[#0099CC]/15 text-[#0099CC] border border-[#0099CC]/20 font-semibold rounded-lg">{item}</span>
                  {i < arr.length - 1 && <Icon name="arrowRight" size={12} className="text-white/30" />}
                </React.Fragment>
              ))}
            </div>
          </div>
      ...
      <section id="roadmap" className="py-20 sm:py-28 bg-[#F8FAFF] relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="max-w-2xl mx-auto text-center mb-16">
            <span className="inline-flex items-center gap-1.5 text-xs font-bold text-[#7C3AED] bg-[#7C3AED]/10 px-3 py-1.5 rounded-full uppercase tracking-widest mb-4">
              <Map size={12} className="text-[#7C3AED]" />
              로드맵
            </span>
            <h2 className="text-3xl sm:text-4xl lg:text-5xl font-extrabold text-[#0D1B2A] mb-4">
              TIKI는 계속 성장합니다
            </h2>
            <p className="text-[#5A6F8A] text-base sm:text-lg">현재 버전은 시작일 뿐, 팀의 더 깊은 니즈를 채우기 위해 진화 중입니다.</p>
          </div>

          <div className="max-w-4xl mx-auto grid grid-cols-1 sm:grid-cols-3 gap-5 relative">
            {roadmap.map((r, i) => (
              <div key={i} className={`relative rounded-2xl border p-6 bg-gradient-to-br ${r.bg} ${r.border} hover:-translate-y-1 transition-all hover:shadow-xl`}
                style={{ boxShadow: `0 0 0 1px ${r.color}20` }}>
                {/* 상태 점 */}
                <div className="flex items-center justify-between mb-5">
                  <div className="flex items-center gap-2">
                    <div className={`w-3 h-3 rounded-full ${r.status === "done" ? "animate-none" : "animate-pulse"}`}
                      style={{ backgroundColor: r.color }} />
                    <span className="text-xl font-extrabold" style={{ color: r.color }}>{r.phase}</span>
                  </div>
      ...
      <section className="py-20 sm:py-28 bg-white border-t border-[rgba(0,100,180,0.08)]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="max-w-2xl mx-auto text-center mb-14">
            <span className="inline-flex items-center gap-1.5 text-xs font-bold text-[#0099CC] bg-[#0099CC]/10 px-3 py-1.5 rounded-full uppercase tracking-widest mb-4">
              <Users size={12} className="text-[#0099CC]" />
              팀 소개
            </span>
            <h2 className="text-3xl sm:text-4xl font-extrabold text-[#0D1B2A] mb-4">네오테크를 만드는 사람들</h2>
          ...
          <div className="grid grid-cols-2 gap-6 max-w-xs mx-auto sm:max-w-2xl sm:grid-cols-4">
            {team.map((m, i) => (
              <div key={i} className="text-center group">
                <div className="w-16 h-16 rounded-2xl mx-auto mb-3 flex items-center justify-center text-lg font-extrabold text-white transition-all group-hover:scale-110 group-hover:shadow-lg"
      ...
      <section id="cta" className="relative py-24 sm:py-32 overflow-hidden">
        ...
        {/* 장식 */}
        <div className="absolute inset-0 opacity-10"
          style={{ backgroundImage: "radial-gradient(circle at 50% 50%, rgba(255,255,255,0.8) 1px, transparent 1px)", backgroundSize: "30px 30px" }} />
        ...
        <div className="relative max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <div className="flex flex-col sm:flex-row justify-center items-center gap-4">
            <a href="#"
              ...
              무료로 시작하기
              <Icon name="arrowRight" size={16} className="text-[#0099CC] transition-transform group-hover:translate-x-1" />
            </a>
            <a href="#"
          ...
          <div className="mt-12 flex flex-wrap justify-center gap-6 text-white/60 text-sm">
            <span className="flex items-center gap-1.5"><Icon name="check" size={14} className="text-[#10B981]" /> 무료 플랜 제공</span>
            <span className="flex items-center gap-1.5"><Icon name="check" size={14} className="text-[#10B981]" /> 설치 불필요</span>
            <span className="flex items-center gap-1.5"><Icon name="check" size={14} className="text-[#10B981]" /> 데이터 보안 보장</span>
          </div>
        </div>
      </section>
