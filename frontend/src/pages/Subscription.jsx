        ...
        return (
            <div className="flex flex-col gap-5">
                {FEATURE_COMPARISON.map((group, gi) => (
                    <div
                        key={group.category}
                        className="overflow-hidden rounded-[14px] border border-[rgba(0,100,180,0.1)] bg-white shadow-[0_2px_12px_rgba(0,100,180,0.06)]"
                    >
                        {/* Category header */}
                        <div
                            className="flex items-center gap-2.5 border-b border-[rgba(0,100,180,0.08)] px-4 py-3"
                            style={{ background: `${CATEGORY_COLORS[gi]}08` }}
                        >
                            <span
                                className="h-2 w-2 rounded-full shrink-0"
                                style={{ background: CATEGORY_COLORS[gi] }}
                            />
                        ...
                        <div className="grid grid-cols-[1fr_1fr_1fr_1fr] border-b border-[rgba(0,100,180,0.06)] px-4 py-2">
                            ...
                            {['free', 'pro', 'team'].map((planId) => (
                                <div
                                    key={planId}
                                    className="flex justify-center rounded-[6px] py-0.5"
                                    style={{
                                        background: PLAN_META[planId].borderTop
                                            ? `${PLAN_META[planId].borderTop}12`
                                            : 'transparent',
                                    }}
                                >
                                    <span
                                        className="text-[11px] font-bold"
                                        style={{ color: PLAN_META[planId].headerColor }}
                                    >
                        ...
                        {group.rows.map((row, i) => (
                            <div
                                key={row.feature}
                                className={cn(
                                    'grid grid-cols-[1fr_1fr_1fr_1fr] items-center px-4 py-3.5',
                                    i !== 0 && 'border-t border-[rgba(0,100,180,0.05)]'
                                )}
                            >
                                ...
                                {['free', 'pro', 'team'].map((planId) => (
                                    <div
                                        key={planId}
                                        className="flex justify-center rounded-[6px] py-1"
                                        style={{ background: PLAN_META[planId].colBg }}
                                    >
                    ...
                    {FEATURE_COMPARISON.map((group, gi) => (
                        <>
                            <tr
                                key={group.category}
                                className={cn(
                                    'border-b border-[rgba(0,100,180,0.06)]',
                                    gi > 0 && 'border-t-2 border-t-[rgba(0,100,180,0.07)]'
                                )}
                            >
...
function FaqItem({ q, a }) {
    ...
    return (
        <div className="border-b border-[rgba(0,100,180,0.08)] last:border-0">
            ...
            {open && <p className="pb-5 text-[14px] leading-relaxed text-[#5A6F8A]">{a}</p>}
        </div>
    );
}
        ...
        if (plan.price.monthly === 0) {
            try {
                ...
                navigate('/subscription/complete', {
                    state: { plan, billing: selectedBilling, subscription, paidAmount: 0 },
                });
            } catch (error) {
        ...
        <div className="min-h-screen bg-[#F8FAFF]">
            ...
            <main className={cn('mx-auto max-w-[1080px]', isMobile ? 'px-4 pb-28 pt-24' : 'px-6 pb-20 pt-28')}>
                ...
                <section className="mb-12 text-center">
                    ...
                    <h1
                        className={cn(
                            'font-bold tracking-[-1.5px] text-[#0D1B2A]',
                    ...
                    <p
                        className={cn(
                            'mx-auto mt-3 text-[#5A6F8A]',
                            isMobile ? 'text-[14px]' : 'text-[16px] max-w-[500px]'
                        )}
                    >
                ...
                <section className={cn('mb-16 grid gap-4', isMobile ? 'grid-cols-1' : 'grid-cols-3')}>
                    {[
                        { icon: 'shield', title: '안전한 결제', desc: 'SSL 암호화로 카드 정보를 안전하게 보호합니다.' },
                        {
                            icon: 'zap',
                            title: '즉시 이용 가능',
                            desc: '결제 완료 후 바로 모든 기능을 사용할 수 있습니다.',
                        },
                        {
                            icon: 'users',
                            title: '언제든지 취소',
                            desc: '약정 없이 언제든지 구독을 변경하거나 취소할 수 있습니다.',
                        },
                    ].map(({ icon, title, desc }) => (
                        <div
                            key={title}
                            className="flex items-start gap-3 rounded-[12px] border border-[rgba(0,100,180,0.08)] bg-white px-5 py-4"
                        >
                ...
                <section className="mb-16">
                    <h2
                        className={cn(
                            'mb-2 font-bold tracking-[-0.5px] text-[#0D1B2A]',
                ...
                <section className={cn('rounded-[20px] bg-[linear-gradient(135deg,#0099CC,#7C3AED)] p-1')}>
                    <div
                        className={cn(
                            'rounded-[18px] bg-[linear-gradient(135deg,rgba(0,153,204,0.95),rgba(124,58,237,0.95))] text-center',
                            isMobile ? 'px-6 py-10' : 'px-12 py-14'
                        )}
                    >
                        <h2
                            className={cn(
                                'font-bold tracking-[-1px] text-white',
